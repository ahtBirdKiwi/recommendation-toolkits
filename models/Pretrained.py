...
class ItemEmbeddings(nn.Module):
    ...
    def __init__(self, item_data, dim_data, settings_params='params/ItemEmbeddings.env',
        ...
        self.levels = self.params['max_score'] if 'walk_length' in self.params.keys(
        ) else 20

        self.walk_length = self.params['walk_length'] if 'walk_length' in self.params.keys(
        ) else 20
        ...
        self.num_workers = self.params['num_workers'] if 'num_workers' in self.params.keys(
        ) else 1
        self.win_size = self.params['win_size'] if 'win_size' in self.params.keys(
        ) else 3
        self.n_iters = self.params['n_iters'] if 'n_iters' in self.params.keys(
        ) else 50
        self.embed_size = self.params['embed_size'] if 'embed_size' in self.params.keys(
        ) else 128
    ...
    def _create_edge_list(self, layer):
        """
            Slice data with level of parameter l and create networkx edge_list
        """
        edge = np.where(self.edges == layer, 1, 0)
        edge = self._confidence(edge)

        edge = pd.DataFrame(edge, index=range(self.n_item),
                            columns=range(self.n_item))
    ...
    def trainings(self, layer, embeddings=None):
        G = self._create_edge_list(l=layer)
    ...
    def positional_embeddings(self):
        ...
        pos_dim, side_dim = int(
            total_embedding_dim / 1.5), total_embedding_dim - int(total_embedding_dim / 1.5)

        emb_layer = nn.Embedding(
            num_embeddings=self.n_item, embedding_dim=pos_dim, sparse=True)
        emb_layer_side = nn.Embedding(
            num_embeddings=self.dim.shape[1] - 1, embedding_dim=side_dim)

        for item in range(self.n_item):
            one_hot = emb_layer(torch.tensor(item))
            side_index = list(
                self.dim[self.dim[self.item_col] == item].iloc[0, :-1]).index(1)
    ...
    def _concat_embeddings(self):
        for item in tqdm(range(self.n_item), desc='Join Pooling Layer with Positional', disable=os.environ.get("DISABLE_TQDM", False)):
            if item in self.pos:
                self.all[item] = np.concatenate(
                    (self.all[item], self.pos[item]))
            else:
                self.all[item] = np.concatenate(
                    (self.all[item], [0] * int(self.embed_size / 2)))

    def fit(self, dump=True):
        self.positional_embeddings()

        self.all = {}
        for layer in tqdm(range(self.levels + 1), desc='Create Item Embeddings...', disable=os.environ.get("DISABLE_TQDM", False)):
            if self.all == {}:
                embeddings = self.trainings(l=layer, embeddings=None)
                self.all = embeddings.copy()
            else:
                self.all = self.trainings(l=layer, embeddings=self.all)

            self.hparam['Social_Relation_Embedding_' +
                        str(layer)] = self.embed_size
            self.hparam['Order'].append('Social_Relation_Embedding_' + str(layer))
...
class UserEmbeddings(nn.Module):
    ...
    def __init__(self, data, settings_params='params/UserEmbeddings.env',
        ...
        self.data = self.data.drop_duplicates(
            subset=[self.user_col, self.item_col, self.day_col])
        ...
        self.data[self.user_col] = [self.uid[x]
                                    for x in self.data[self.user_col]]
        self.data[self.item_col] = [self.iid[x]
                                    for x in self.data[self.item_col]]
    ...
    def _create_user_sequences(self, layer=-1, padding=False, all_item=False):
        if not all_item:
            data = self.data[self.data[self.rating_col] == layer]
        else:
            data = self.data.copy()
        ...
        for user, df in data.groupby(by=self.user_col):
            if all_item:
                seq[user] = [int(x) for x in list(
                    df[self.item_col] * df[self.rating_col])]
            else:
    ...
    def _trainings(self, layer, embeddings=None):
        seq = self._create_user_sequences(layer, padding=False)
    ...
    def w2v_embeddings(self):
        self.all = {}
        for layer in tqdm(range(self.levels + 1), desc='Create User Embeddings...', disable=os.environ.get("DISABLE_TQDM", False)):
            if self.all == {}:
                embeddings = self._trainings(l=layer, embeddings=None)
                self.all = embeddings.copy()
            else:
                self.all = self._trainings(l=layer, embeddings=self.all)
    ...
    def _poolings(self, layer):
        data = self.data[self.data[self.rating_col] == layer]

        part_pre = int(self.embed_size * layer)
        part_last = int(self.embed_size * (layer + 1))
        ...
        for user in range(self.n_user):
            ...
            if len(set(df[self.item_col])) > 0:
                pool = np.zeros(
                    shape=(len(set(df[self.item_col])), self.embed_size))
                ...
                avg_pool, mean_pool = np.zeros(
                    shape=(self.embed_size)), np.zeros(shape=(self.embed_size))

            if user in self.embeddings:
                self.embeddings[user] = np.concatenate(
    ...
    def _concat_embeddings(self):
        for user in tqdm(range(self.n_user), desc='Join Pooling Layer with Positional', disable=os.environ.get("DISABLE_TQDM", False)):
            if user in self.pos:
                self.embeddings[user] = np.concatenate(
                    (self.embeddings[user], self.pos[user]))
            else:
                self.embeddings[user] = np.concatenate(
                    (self.embeddings[user], [0] * self.embed_size))
    ...
    def fit(self, dump=True):
        ...
        for layer in tqdm(range(self.levels + 1), desc='Pooling User Embeddings ...', disable=os.environ.get("DISABLE_TQDM", False)):
            self.embeddings = self._poolings(layer)

            self.hparam['Order'].append('Item_Poolings_' + str(layer))
            self.hparam['Item_Poolings_' + str(layer)] = self.embed_size * 2

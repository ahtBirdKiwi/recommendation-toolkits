        ...
        for word in range(self.n_item):
            try:
                if type(pre_trained) != dict:
                    ...
                    self._embeddings[word] = np.concatenate(
                        (self._embeddings[word], self.w2v_model.wv[str(word)]))
...
class __baseGRU4REC(pl.LightningModule):
    ...
    def __init__(self, data, gru_pre_embedding_size=64, gru_hidden_dims=20, num_gru_layers=3, embed_size=32):
        super(__baseGRU4REC, self).__init__()
        ...
        self.emb = nn.Embedding(num_embedding=self.num_embeddings,
                                embedding_size=self.gru_pre_embedding_size, sparse=True)
        self.gru = nn.GRU(input_size=self.gru_pre_embedding_size,
                          hidden_size=self.gru_hidden_dims, num_layers=self.num_gru_layers)

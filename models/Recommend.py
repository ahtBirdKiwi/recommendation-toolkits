...
class BuildModel():
    def __init__(self, train, val, uid, iid, test=None, model_type='NeuCF', if_production=False,
        ...
        self.batch_size = self.params['batch_size'] if 'batch_size' in self.params.keys(
        ) else 512
        self.input_dim = self.params['input_dim'] if 'input_dim' in self.params.keys(
        ) else 128
        self.hidden_layer_dims = eval(self.params['hidden_layer_dims']) if 'hidden_layer_dims' in self.params.keys() else [
            self.input_dim, self.input_dim // 2]
        self.max_epochs = self.params['max_epochs'] if 'max_epochs' in self.params.keys(
        ) else 10
        self.patience = self.params['patience'] if 'patience' in self.params.keys(
        ) else self.max_epochs // 3
        self.min_delta = self.params['min_delta'] if 'min_delta' in self.params.keys(
        ) else 1e-5
        self.class_weight = eval(self.params['class_weight']) if 'class_weight' in self.params.keys(
        ) else [1] * len(self.hidden_layer_dims)
        self.dropout = self.params['dropout'] if 'dropout' in self.params.keys(
        ) else 0.3
        self.output_thres = self.params['output_thres'] if 'output_thres' in self.params.keys(
        ) else 0.5
        self.lr_rate = self.params['lr_rate'] if 'lr_rate' in self.params.keys(
        ) else 0.5
    ...
    def fit(self, save=True):
        ...
        self.trainer = pl.Trainer(max_epochs=self.max_epochs, callbacks=[
                                  self.early_stop_callback, self.checkpoint_callback])
        self.trainer.fit(self.model, self.dataset)

        if save:
            self.trainer.save_checkpoint(
                self.save_path + self.model_type + '.ckpt')
    ...
    def predict_all(self, top_n=30):
        ...
        user_indexing = self.test.groupby(by=self.user_col)[
            self.item_col].count().reset_index()
    ...
    def validate_user(self, scaledown=True):
        ...
        for i in tqdm(range(tester.shape[0]), desc='Validating ... ', disable=os.environ.get("DISABLE_TQDM", False)):
            ...
            user, item, y = tester.iloc[i,
                                        0], tester.iloc[i, 1], tester.iloc[i, -1]
        ...
        for item in set(res['Real']):
            sns.displot(data=res[res['Real'] == item],
                        x='Predicted', kind='kde')
